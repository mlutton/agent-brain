...
- **Publication (C8).** Read uses one publication verdict per path. Rule 1 makes a path named by an open intent `unpublished`, including a deleted path. Rule 2 keeps a retained original or attachment's recorded role and reports a missing file as `retained_missing`. Rule 5 makes an unrecorded `type: document` file `unverified` with `reason: no_ingest_record`. For recorded documents, rule 5 also requires current committed bytes to descend from a consistent ingest or adopt record through consistent `Brain-Op: update` writes; an out-of-band committed change reads `unverified` with `reason: no_ingest_record`, before parsing even malformed bytes. Rule 6 makes a file referenced by an unverified document `unverified` with `hint: suspected_ingestion`, whether the referrer is nearby or elsewhere in a scanned zone; a hidden name is not scanned and so never refers. Rule 7 leaves an uncommitted edit `ok` with `labels: {"changed_out_of_band": true}` while the journal is present, and makes it `unverified` with `reason: journal_missing` when the journal is missing. A deleted path without an intent or retained record is `not_found`. `unverified` withholds content unless `include_unverified` is `true`. If git cannot be consulted, or the root is not a git repository, the answer is `unverified` with `reason: git_unavailable`, never `ok`.

  This slice changes read in three deliberate ways. First, recorded documents whose committed bytes do not descend through brain updates now read `unverified` / `no_ingest_record`. Second, a deleted path named by an open intent reads `unpublished` instead of `not_found` (a deleted retained path named by that intent also reads `unpublished`; on the base it read `ok` with `integrity: retained_missing`). Third, rule 5 runs before rule 6: an unrecorded `type: document` named through `original` or `attachments` by another unrecorded document now reads `unverified` / `no_ingest_record`; previously it read `unverified` with `hint: suspected_ingestion`.
...
**Honest limits, as shipped today.** Rule 3 is not delivered: a moved copy of a retained file can read as published. Rule 4 is not delivered. Rule 8 is partly delivered through its referrer signal only; the raw-item-bytes signal is reachable today and not delivered, nor are the item-folder or `Brain-Unverified` signals. Rule 1's run-manifest half is not delivered. The following interpretations remain open for later work: whether audit commits extend descent and working-copy edits to recorded documents, both owned by ST-09; merge lineage, recorded role versus current `type`, whether an update's `prior` must chain, descent before parse, and which publication record is latest. For the latest-record question, the latest record means the newest consistent `role=document` publication record in `git log` order, not the record on HEAD's path lineage. In the rare case of a side line re-ingesting the path and then being merged back with its bytes discarded (`merge -s ours`), the document reads `unverified`. Under clock skew, commit-date order can differ from ancestry order. Tests pin the present behavior of audit commits, working-copy edits to recorded documents, merge lineage, recorded role versus current `type`, whether an update's `prior` must chain, and descent before parse; the latest-record question records the current lookup choice. Also not delivered: `support` (its key is absent); `heading` (a request carrying the key at all, with any value, is refused `unsupported_heading`); every `labels` key but `changed_out_of_band`; the content fields of a `retained_missing` path (only the role, owner, integrity, recorded hash and publication commit are reported); request validation (a request without an integer `max_bytes` fails as an unhandled error, exit `1`); precedence beyond the documented cases; whether `invalid` carries an errors list; and any test that read changes nothing on disk. The key names `integrity`, `recorded_sha256`, `publication_commit`, `owner` (as a path), `reason`, `hint` and `labels` (as an object) are this implementation's choice: the specification lists no key for them. `read` and `validate` disagree about retained paths today: `validate`'s `retained` count is always `0`, while `read` reports a path with a record as `role: original` or `attachment`.
