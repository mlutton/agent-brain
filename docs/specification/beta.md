...
3. Write `.brain/setup.json`: `brain_id`, `starter_url`, `commit`, `created`, `modules`, and `copied: [{path, sha256}]`. `starter_url` is the first configured fetch URL of the starter's `origin` remote when one exists. Otherwise it is the canonical absolute `file:` URI of the starter checkout. Other named remotes do not affect this value. Determining the value performs no network access. This is source-location metadata, not proof of authenticity or future accessibility; the pinned starter commit and copied-file hashes remain recorded as specified.
4. Write `.gitignore` containing the journal and cache folders. With `--git`, initialise a repository and make the first commit with trailer `Brain-Op: setup`. With `--git`, setup generates a run identifier for the initial commit. Its final trailer paragraph includes `Brain-Op: setup` and `Brain-Run: <run_id>`. The identifier is generated for this setup invocation; this does not require a persist intent, run manifest, or setup recovery protocol. Other C7 trailer conditions remain unchanged.
...
A successful setup returns exactly one JSON object with `"outcome": "setup_complete"` and exits `0`. A refused request returns `"outcome": "refused"` with a `reason`, exits `2`, and changes nothing, as in C1. A failed smoke check returns `"outcome": "setup_failed"` with a `check` naming the failed check and exits `3`. The hard-link check identifiers are `hardlink_data` and `hardlink_state`. C1's unhandled-error and `vendored_dependency` rules remain unchanged. This defines neither a transactional rollback guarantee nor a classification of every possible copy or Git failure.
...
Setup installs the skill files and selected module files supplied by the named clean starter commit; installation does not author their contents. ST-04 delivers and verifies those installation mechanisms using nonempty synthetic skill and module payloads in clean, committed starter fixtures. Skill files are copied into both `.claude/skills/` and `.agents/skills/`, and every copied destination and hash is recorded. Selected-module installation copies the supplied manifest and type definitions, creates the declared data folder, and records the installed modules and copied-file hashes. An absent requested module is refused before target changes. Before changing the target, setup checks the selected module manifest's `folder` using the same admissible-folder rules as module detection: a nonempty string naming a single, non-hidden folder, excluding `.` and `..` and the reserved base zones `inbox`, `raw`, `documents`, and `wiki`. A missing or invalid `folder` is refused with `outcome: refused`, `reason: invalid_module_folder`, exit 2, with no target or surrounding filesystem changes. Setup never creates a module data folder outside the target. This adds no general malformed-manifest schema, rollback, or recovery contract. ST-08 supplies the real projects manifest/type definitions and verifies their validation, persistence and discovery integration through setup. ST-14 supplies the operational skills, including the setup skill, and verifies their fresh-agent behavior. No setup-only production skill is required in ST-04; an empty production skill set must be reported honestly and does not establish delivery of the ST-14 experience. No additional public module names or general plugin registry are required.
...
| `target_unexpected` | `expected_prior`, `intended_sha256`, `observed_sha256` (a hash, `absent`, or `null` where `reason` is not `null`), `backup_path`, `reason` | **open**; nothing written |
...
- **`target_unexpected` is not resolved by recovery.** It classifies "the tool could not establish what happened", so the intent stays open, nothing is overwritten, and no change record or commit is made. Every later recovery reports it again, and Behaviour 21 keeps the path locked against `persist` until a person acts. An intent that cannot be trusted at all — unreadable, missing a field C6 step 2 records, or naming a temp path that is not a persist temp file inside the brain — is reported the same way, with `reason: "invalid_intent"`, `observed_sha256: null` and any field it does not carry as `null`; recovery acts on nothing it names, and the temp file it removes is the one it checked, never another that a link resolves to. `reason` is on every `target_unexpected` entry, never omitted, so every entry of this classification carries the same keys. It is `null` for an ordinary one, and otherwise one of three values, each with `observed_sha256: null` because no hash was established: `"invalid_intent"` as above; `"not_a_regular_file"`, when the target is a link, a folder or another file that is not a regular file, or lies outside the brain — recovery hashes only a regular file inside the brain, so it hashes nothing else, and makes no change record or commit for it; and `"recovery_failed"`, when an unexpected error (an unreadable target, a journal folder that refuses changes) stopped this intent's recovery. That failure is the intent's own: every other intent is still recovered and reported, what this one had already finished stays finished and is not repeated, and the intent stays open for a later recovery.
...
Setup also recognizes `BRAIN_FAULT=setup_hardlink_data:fail` and `BRAIN_FAULT=setup_hardlink_state:fail` only when `BRAIN_TEST_FAULTS=1`. Each simulates an unsupported-hard-link error at the corresponding setup smoke check's link-creation operation and exercises the same error-handling path as an actual hard-link creation failure. The resulting check identifiers are `hardlink_data` and `hardlink_state`, respectively. Without the test gate, these values have no effect. These setup points support `fail` only; no setup kill/recovery contract is introduced. Ordinary setup still exercises real hard-link creation and refusal to replace an existing link in both data and state locations. Tests use the real filesystem and Git without mocking either; no separate fault framework is introduced.
