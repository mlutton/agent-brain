...
- `update`: applies a targeted, byte-preserving replacement of only the named properties' spans (C4) — comments, hand ordering and untouched properties (including zero-indented block sequences) are left exactly as they were; refuses `unsupported_frontmatter` byte-identical when a touched span holds a comment or the block uses anchors, aliases, explicit tags, multiple YAML documents or flow style; refuses `version_mismatch` when `expected_version` no longer matches the current file. A targeted-replacement `update` whose resulting bytes would equal the target's current bytes is refused before anything is written — `version_mismatch` when `expected_version` is stale, otherwise `no_change` — so it can never leave a path locked by an intent with no way to close it ([#38](https://github.com/mlutton/agent-brain/issues/38), [#42](https://github.com/mlutton/agent-brain/issues/42)). `update` can only replace an existing property's span — it cannot add a property the document does not already carry, and refuses `unsupported_frontmatter` byte-identical for one; appending a new property is out of this story's scope.
...
**Honest limits, as shipped today:** `no_change` is a `persist` refusal reason this implementation adds; the specification's C6 outcome table does not name it, and C6 is not amended by this change — this is recorded here as an open question for the specification to gain. `persist` never retries after an unexpected target, and its own `target_unexpected` outcome cannot be reached through the command line today ([#37](https://github.com/mlutton/agent-brain/issues/37)); the state it stands for is observed from `recover`, below. Index bookkeeping (C8) is ST-06's; `written_incomplete`'s pending list never names `index` before then. `brain grant`, maintenance-grant validity and invalidation are out of scope; persist only records a create-time grant trailer for an agent-authored document. `discover`, `audit` and `ingest` do not exist yet, so nothing published by persist is findable except through `read` by id or path, or by reading the brain's files directly.
...
- **Publication (C8).** Read applies rule 1 (a file named by an open intent is `unpublished`: no content, whatever `include_unverified` says, decided before the file is parsed), rule 2 (a path with a publication record keeps its role whatever its bytes, and its record, never its bytes, is also what keeps it from being the referrer rule 6 looks for), the first half of rule 5 (a `type: document` file with no ingest or adopt record is `unverified` with `reason: no_ingest_record`; the other half, that a recorded document's current bytes still descend from that record, is not applied, see the limits below), rule 6 (a file named by such a document, wherever in the scanned zones that document sits, through `original` or `attachments` is `unverified` with `hint: suspected_ingestion`; a hidden name is not scanned and so never refers) and rule 7 (committed is published; uncommitted with the journal present is `ok` with `labels: {"changed_out_of_band": true}`; uncommitted with the journal gone is `unverified` with `reason: journal_missing`). `unverified` withholds content unless `include_unverified` is `true`. If git cannot be consulted, or the root is not a git repository, the answer is `unverified` with `reason: git_unavailable`, never `ok`.
