...
- **Publication (C8).** Read uses one publication verdict per path. Rule 1 makes a path named by an open intent `unpublished`, including a deleted path. Rule 2 keeps a retained original or attachment's recorded role and reports a missing file as `retained_missing`. Rule 5 makes an unrecorded `type: document` file `unverified` with `reason: no_ingest_record`. For recorded documents, rule 5 also requires current committed bytes to descend from a consistent ingest or adopt record through consistent `Brain-Op: update` writes; an out-of-band committed change reads `unverified` with `reason: no_ingest_record`, before parsing even malformed bytes. Rule 6 makes a file referenced by an unverified document `unverified` with `hint: suspected_ingestion`, whether the referrer is nearby or elsewhere in a scanned zone; a hidden name is not scanned and so never refers. Rule 7 leaves an uncommitted edit `ok` with `labels: {"changed_out_of_band": true}` while the journal is present, and makes it `unverified` with `reason: journal_missing` when the journal is missing. A deleted path without an intent or retained record is `not_found`. `unverified` withholds content unless `include_unverified` is `true`. If git cannot be consulted, or the root is not a git repository, the answer is `unverified` with `reason: git_unavailable`, never `ok`.
  ...
  This slice changes read in three deliberate ways. First, recorded documents whose committed bytes do not descend through brain updates now read `unverified` / `no_ingest_record`. Second, a deleted path named by an open intent reads `unpublished` instead of `not_found` (including a retained path named by that intent). Third, rule 5 runs before rule 6: an unrecorded `type: document` named through `original` or `attachments` by another unrecorded document now reads `unverified` / `no_ingest_record`; previously it read `unverified` with `hint: suspected_ingestion`.
