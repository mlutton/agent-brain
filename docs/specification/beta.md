...
- **Publication rule** (git brains). discover, read, rebuild, validate's `retained` status and audit all apply the same rule. The first matching rule wins.
  ...
  6. **Every other data-zone file**, Markdown or not (notes, wiki pages, module documents, unmanaged notes, non-Markdown files):
  ...
  7. **`suspected_ingestion` hint.** Added to an `unverified` file when a surviving durable signal ties it to ingestion: an uncommitted `document` references it; its bytes equal a current `raw/` item (raw items are deleted only after a run commit); it sits in an item folder with such a file; or a `Brain-Unverified` trailer lists it.
...
- **Journal missing.** While the journal is not present, operations that write refuse with `journal_missing`. The user resolves this with `brain recover --start-journal`, an explicit human step that:
  ...
  After the reset, those paths stay `unverified` (rule 3) until resolved, including if they are edited again. New uncommitted files created after the reset at other paths are ordinary `changed_out_of_band` changes.
