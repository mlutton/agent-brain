...
- **Publication (C8).** Read uses one publication verdict per path. Rule 1 makes a path named by an open intent `unpublished`, including a deleted path. Rule 2 keeps a retained original or attachment's recorded role and reports a missing file as `retained_missing`. Rule 5 requires a document's current committed bytes to descend from a consistent ingest or adopt record through consistent `Brain-Op: update` writes; an out-of-band committed change reads `unverified` with `reason: no_ingest_record`, before parsing even malformed bytes. Rule 6 makes a file referenced by an unverified document `unverified` with `hint: suspected_ingestion`, whether the referrer is nearby or elsewhere in a scanned zone. Rule 7 leaves an uncommitted edit `ok` with `labels: {"changed_out_of_band": true}` while the journal exists, and makes it `unverified` with `reason: journal_missing` when the journal is gone. A deleted path without an intent or retained record is `not_found`. `unverified` withholds content unless `include_unverified` is `true`. If git cannot be consulted, or the root is not a git repository, the answer is `unverified` with `reason: git_unavailable`, never `ok`.
