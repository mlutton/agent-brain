...
class TestExitCodesAndRefusals(TempBrainTestCase):
    ...
    def test_known_but_not_implemented_subcommand_is_refused(self):
        # `persist` (ST-02) and `recover` (ST-03) now exist, so neither is an
        # example of a known-but-unimplemented subcommand (see the story's
        # amendment B3); `rebuild` still is, per C1's subcommand list and
        # bin/brain's dispatch, so it keeps this generic dispatch behaviour
        # under test.
        proc = support.run_brain(["rebuild", "--root", self.tmpdir])
        report = support.parse_single_json(proc.stdout)
        self.assertEqual(proc.returncode, 2)
